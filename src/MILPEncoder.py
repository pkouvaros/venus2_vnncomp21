...
from src.Layers import Input, FullyConnected, Conv2D, GlobalAveragePooling
...
class MILPEncoder:
    ...
    def __init__(self, prob, logfile, intra=True, inter=True):
        """
        Arguments:
            ...
            intra: Whether to use intra-layer dependency constraints.

            inter: Whether to inter-layer dependency constraints. 
        """

        self.prob = prob
        self.logfile = logfile
        # if MILPEncoder.logger is None:
            # MILPEncoder.logger = get_logger(__name__, logfile)
        self.intra = intra
        self.inter = inter
    ...
    def encode(self):
        ...
        if self.intra or self.inter:
            dg = DependencyGraph(self.prob.spec.input_layer,
                                 self.prob.nn, 
                                 self.logfile, 
                                 self.intra, 
                                 self.inter)
