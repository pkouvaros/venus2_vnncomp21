...
from src.DependencyGraphNode import DependencyGraphNode
from src.utils.DependencyType import DependencyType
...
class DependencyGraph:
    ...
    def __init__(self, input_layer, nn, logfile, intra=True, inter=True):
        """
        Arguments:
            ...
            inter: bool. Whether or not to use inter-layer dependencies for the
            construction of the dependency graph.

            intra: bool. Whether or not to use intra-layer dependencies for the
            construction of the dependency graph.
        """
    ...
    def build(self, delta_vals=None):
        ...
        # print('Finished building dependency graph, time: {:.2f}, '
              # '#intra deps: {}, ' 
              # '#inter deps: {}.'.format(te-ts, 
                                        # self.intra_deps_count, 
                                        # self.inter_deps_count))
        # self.logger.info('Finished building dependency graph, time: {:.2f}, '
                         # '#intra deps: {}, ' 
                         # '#inter deps: {}.'.format(te-ts, 
                                                   # self.intra_deps_count, 
                                                   # self.inter_deps_count))
    ...
    def _layer_intra_deps(self, p_l, l, v=None):
        ...
        Returns:
            ...
            None
        """ 
        for (i,j) in list(itertools.combinations(l.outputs(),2)):
