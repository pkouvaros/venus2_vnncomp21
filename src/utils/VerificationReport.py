...
class VerificationReport:
    def __init__(self, logfile, result=SolveResult.UNKNOWN, cex=None, runtime=0):
        """
        Arguments:
            
            logfile: str of path to logfile

            result: SolveResult (result from MILPSolver)

            cex: np.array of a counter-example

            runtime: double (total verification time)
    ...
    def process_solve_report(self, report):
        ...
        if report.result == SolveResult.BRANCH_THRESHOLD:
            ...
            self.cex = report.cex
        elif report.result == SolveResult.SATISFIED:
            self.finished_jobs_count += 1
